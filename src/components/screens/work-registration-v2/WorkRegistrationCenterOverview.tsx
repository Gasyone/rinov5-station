...
interface WorkRegistrationCenterOverviewProps {
  ...
  onOpenBranchDay: (branch: string, date: string, dayLabel: string) => void
}
  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col gap-2">
      <div className="grid shrink-0 gap-2 md:grid-cols-2 xl:grid-cols-4">
        <MetricTile compact label="Trường có đăng ký" value={registeredBranches} icon={Building2} />
      ...
      <div className="flex h-full min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-border bg-card">
        <div className="min-h-0 flex-1 overflow-auto">
          {paged.length > 0 ? (
            <Table>
              <TableHeader className="[&_tr]:border-b-0">
                <TableRow className="border-b-0 bg-muted/50">
                  <TableHead className="sticky left-0 z-10 bg-muted/50">Trường</TableHead>
            ...
            <EmptyState
              className="h-full"
              title="Chưa có dữ liệu trường"
              description="Không có trường trong phạm vi hiện tại."
            />
          )}
        </div>
