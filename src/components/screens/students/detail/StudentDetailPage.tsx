...
import { EmptyState, AppAvatar, StudentHeaderInfoCard, type ParentMemberInfo } from '@/components/shared'
...
export function StudentDetailPage({
  ...
  const handleSaveLevel = (newLevel: string, newSubLevel: string, newSchoolClass?: string, newEngName?: string) => {
    ...
    toast.success('Cập nhật trình độ học tập thành công!')
  }
  ...
  const handleSaveEnglishName = (newEngName: string) => {
    ...
    toast.success('Đã lưu tên tiếng Anh!')
  }
  ...
  const selectedPkg = useMemo(() => {
    if (selectedPackageId === 'all') return packagesList[0] || null
  ...
  const student = useMemo(() => {
    ...
    return mockStudents.find((s) => s.id === cleanId || s.id === studentId) || mockStudents[0]
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [studentId, revision])

  useEffect(() => {
    if (student) {
      setCustomHeaderTitle(`Học viên: ${student.name} (${student.id})`)
    }
    return () => {
      setCustomHeaderTitle(null)
    }
  }, [student, setCustomHeaderTitle])
  ...
  const activeClass = useMemo(() => {
    ...
    return (
      filteredEnrolledClasses.find((c) => c.status === 'active') ||
      filteredEnrolledClasses[0] ||
      null
    )
  }, [filteredEnrolledClasses, student])

  const parentMembers = useMemo<ParentMemberInfo[]>(() => {
    if (!student) return []
    const members = getStudentFamilyMembers(student)
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-background">
      ...
      <div className="flex items-center gap-3 border-b border-border/80 px-4 py-2.5 bg-card/60 shrink-0">
        <Button
          variant="outline"
          size="icon"
          className="h-8 w-8 rounded-full border-border/70 hover:bg-muted text-muted-foreground hover:text-foreground cursor-pointer"
          onClick={onBack}
        ...
        <div>
          <h2 className="text-sm font-bold text-foreground flex items-center gap-2">
            <span>Chi tiết học viên: {student.name}</span>
            <span className="font-mono text-xs text-primary font-semibold">({studentCode})</span>
          </h2>
        </div>
      </div>

      {/* Split Body Layout */}
      <div className="grid min-h-0 flex-1 gap-4 p-4 lg:p-5 lg:grid-cols-[1fr_420px] overflow-hidden">
        ...
        <main className="flex min-h-0 flex-col overflow-hidden space-y-3">
          ...
          {/* Main Tabs Section: Xếp lớp & Nhật ký */}
