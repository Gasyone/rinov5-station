...
import { StudentDetailPackages } from './StudentDetailPackages'
import { StudentDetailClasses } from './StudentDetailClasses'
import { StudentDetailTickets } from './StudentDetailTickets'
...
import type { StudentNote, StudentGlobalLog } from './studentDetailTypes'
...
export function StudentDetailDialog({
  ...
  // Get student packages & global timeline logs
  const packages = useMemo(() => (student ? getStudentPackages(student) : []), [student])
    ...
    setSideLogs((prev) => [
      {
        ...
        action: `Đã cập nhật thông tin chi tiết học viên.`,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="grid h-[90vh] max-h-[900px] grid-rows-[auto_minmax(0,1fr)] gap-0 overflow-hidden p-0 sm:max-w-[95vw] lg:max-w-[1380px] bg-background">
                    ...
                    <>
                      ...
                      {onCreateTicket && (
                        <Button
                          ...
                          className="rounded-lg text-amber-600 hover:text-amber-700 hover:bg-amber-50 border-amber-200"
                        >
                          <LifeBuoy className="h-4 w-4 mr-1.5" /> Tạo Ticket
                        </Button>
                      )}
                      ...
                      >
                        <Pencil className="h-4 w-4 mr-1.5" /> Chỉnh sửa hồ sơ
                      </Button>
                    </>
                  )}
                </div>
        ...
        {/* 2. Split Body Layout (70% Left Tabs / 30% Right Interaction Notes) */}
