...
export function BookingTestTableRow({
  ...
  const isCheckedIn = booking.attendance === 'confirmed'
  const rowHighlightClass = isCheckedIn
    ? "bg-[#f2fcf7] dark:bg-[#091b11] group-hover:bg-[#e6faf0] dark:group-hover:bg-[#0f2e1e]"
    : "bg-background group-hover:bg-muted"

  return (
    <TableRow
      className={cn("group cursor-pointer border-b-0 transition-colors", isCheckedIn && "bg-emerald-50/10 dark:bg-emerald-950/5")}
      onClick={() => onRowClick(booking.id)}
    >
      <TableCell
        className={cn("sticky left-0 z-30 w-12 min-w-12 max-w-12 overflow-hidden text-center transition-colors", rowHighlightClass, isCheckedIn && "border-l-4 border-l-emerald-500")}
        ...
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              {isCheckedIn && (
                <span className="shrink-0 inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-400 border border-emerald-200/50 font-semibold" title="Đã check-in">
                  <UserCheck className="h-3 w-3 text-emerald-500 fill-emerald-500/10" />
                  Đã đến
                </span>
              )}
            </div>
