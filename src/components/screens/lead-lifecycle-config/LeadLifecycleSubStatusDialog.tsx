...
interface LeadLifecycleSubStatusInnerProps {
  stage: PipelineStageConfig | null
  initialSubStatus?: PipelineSubStatusConfig | null
  onSaveSubStatus: (stageId: string, subStatus: PipelineSubStatusConfig) => void
  onDeleteSubStatus?: (stageId: string, subStatusId: string) => void
...
const LeadLifecycleSubStatusInner: React.FC<LeadLifecycleSubStatusInnerProps> = ({
  stage,
  initialSubStatus,
  onSaveSubStatus,
  onDeleteSubStatus,
  ...
  const [name, setName] = useState(initialSubStatus?.name ?? '')
  const [code, setCode] = useState(initialSubStatus?.code ?? '')
  const [color, setColor] = useState(initialSubStatus?.color || stage?.color || '#3b82f6')
  const [requiresNote, setRequiresNote] = useState(Boolean(initialSubStatus?.requiresNote))
  const [suggestsCallback, setSuggestsCallback] = useState(Boolean(initialSubStatus?.suggestsCallback))
  const [description, setDescription] = useState(initialSubStatus?.description ?? '')
...
export const LeadLifecycleSubStatusDialog: React.FC<LeadLifecycleSubStatusDialogProps> = ({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      {open && (
        <LeadLifecycleSubStatusInner
          key={`${stage?.id || ''}-${initialSubStatus?.id || 'new'}`}
          stage={stage}
          initialSubStatus={initialSubStatus}
