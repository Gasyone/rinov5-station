...
export function TrialClassAssignDialog({
  ...
  const canConfirm = selectedSessions.length > 0 && (!isReschedule || rescheduleReason !== '')

  const handleConfirm = () => {
    if (!canConfirm || !trial) return
    onAssign(trial.id, selectedSessions, internalNotes, isReschedule ? rescheduleReason : undefined)
  }
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent className="max-h-[90vh] flex flex-col overflow-hidden sm:max-w-3xl">
        <DialogHeader className="shrink-0">
          ...
          <DialogDescription>
            {isReschedule
              ? `Xếp lại lịch cho ${trial.studentName} — ${trial.trialName}`
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pr-2">
          ...
          {(selectedSessions.length > 0 || isReschedule) && (
        ...
        <DialogFooter className="shrink-0 flex items-center justify-between sm:justify-between border-t pt-4">
