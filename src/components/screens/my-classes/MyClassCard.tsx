        ...
        <div className="space-y-0.5">
          <div className="flex items-center justify-between gap-1.5 flex-wrap">
            <div className="flex items-center gap-1 min-w-0">
              <span className="font-mono text-xs font-semibold text-muted-foreground bg-muted/60 px-1.5 py-0.2 rounded border border-border/40 shrink-0">
                {cls.code}
              </span>
              <Badge variant="outline" className="text-xs font-bold px-1.5 py-0 h-4 shrink-0">
                {cls.level}
              </Badge>
              {cls.subLevel && (
            ...
            <StatusBadge status={cls.status} label={CLASS_STATUS_LABELS[cls.status]} className="shrink-0 text-xs px-1.5 py-0.2" />
          </div>
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              {teachers.length === 1 ? (
                ...
                <PersonnelHoverCard person={teachers[0]} align="end">
                  <div className="flex items-center gap-1 text-xs font-semibold text-foreground/90 hover:text-primary transition-colors cursor-pointer max-w-[140px] truncate">
          ...
          <div className="space-y-1 pt-0.5 border-t border-border/40">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-semibold text-foreground">
                ...
                <span className={`text-xs ${capacityPct >= 90 ? 'text-destructive font-bold' : 'text-muted-foreground'}`}>
                  ({capacityPct}%)
                </span>
              </span>
            </div>
          ...
          <div className="rounded-md border border-border/50 bg-muted/20 p-1.5">
            <div className="grid grid-cols-3 gap-1 text-center divide-x divide-border/40">
              <div className="px-1 space-y-0.2">
                <span className="text-xs text-muted-foreground block font-medium">Chuyên cần</span>
                <span className="font-bold text-emerald-600 dark:text-emerald-400 text-xs">
                  {academicStats.attendanceRate}%
                </span>
              </div>
              <div className="px-1 space-y-0.2">
                <span className="text-xs text-muted-foreground block font-medium">BTVN</span>
                <span className="font-bold text-sky-600 dark:text-sky-400 text-xs">
                  {academicStats.homeworkRate}%
                </span>
              </div>
              <div className="px-1 space-y-0.2">
                <span className="text-xs text-muted-foreground block font-medium">Điểm TB</span>
            ...
            {academicStats.specialStudents.map((st) => (
              <StudentCareHoverCard key={st.id} student={st}>
                <div className="relative cursor-pointer transition-transform hover:scale-105 shrink-0">
                  ...
                  {st.type === 'at_risk' && (
                    <span className="absolute -top-0.5 -right-0.5 flex h-3 w-3 items-center justify-center rounded-full bg-destructive text-xs text-white font-bold ring-2 ring-background">
                      !
                    </span>
                  )}
                </div>
