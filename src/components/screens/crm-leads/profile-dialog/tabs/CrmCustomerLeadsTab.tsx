...
import type { CustomerProfileLeadOccurrence } from '../crmCustomerProfileTypes'

interface CrmCustomerLeadsTabProps {
  occurrences: CustomerProfileLeadOccurrence[]
  onReactivateLead?: () => void
}

export function CrmCustomerLeadsTab({
  occurrences,
  onReactivateLead,
}: CrmCustomerLeadsTabProps) {
  return (
    <div className="space-y-3.5">
      {/* Header Bar với thống kê & nút hành động */}
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <h4 className="text-xs font-bold text-foreground">
              Lịch sử các đợt tiếp cận Lead ({occurrences.length} đợt)
            </h4>
            <p className="text-[11px] text-muted-foreground">
              Theo dõi toàn bộ các lần phát sinh nhu cầu học tập và chu kỳ tư vấn bán hàng
            </p>
          </div>
        </div>
      ...
      {/* Danh sách các lần tiếp cận dạng Timeline Cards */}
