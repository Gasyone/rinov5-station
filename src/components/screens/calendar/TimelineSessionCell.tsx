...
export function TimelineSessionCell({
  ...
  if (!sessions || sessions.length === 0) {
    return (
      <div className="h-10 flex items-center justify-center text-xs text-muted-foreground/30 italic select-none">
        —
      </div>
    )
  }
    ...
    return (
      <SessionHoverCard session={session}>
        <div
          ...
          className={cn(
            "group relative flex flex-col justify-center gap-1 overflow-hidden rounded-md p-1.5 transition cursor-pointer border text-xs leading-tight select-none min-h-[44px] w-full",
            bgClass
          )}
        >
          ...
          <div className={cn("flex items-center justify-between gap-1 text-xs", session.isOpeningDay && "pl-1")}>
            ...
            <div className="flex items-center gap-0.5 font-bold text-foreground/90 shrink-0">
              ...
              {session.trialStudents > 0 && (
                <span className="text-xs text-violet-600 dark:text-violet-400 font-semibold" title={`${session.trialStudents} học thử`}>
    ...
    <TimelineGroupHoverCard sessions={sessions} onSelectSession={onSelectSingle}>
      ...
      >
        ...
        <div className="flex items-center justify-between gap-1 text-xs">
          ...
          <span className="inline-flex items-center gap-0.5 rounded-full bg-indigo-200/80 dark:bg-indigo-900/80 border border-indigo-300 dark:border-indigo-700 px-1.5 py-0.2 text-xs font-bold text-indigo-800 dark:text-indigo-200 shrink-0">
        ...
        <div className="flex items-center justify-between text-[9.5px] text-muted-foreground">
          <span className="text-xs font-medium text-foreground/90 truncate flex-1 min-w-0" title={`Giáo viên: ${teacherSummary}`}>
            {teacherSummary}
          </span>
