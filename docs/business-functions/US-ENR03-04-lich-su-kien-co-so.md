...
- **Mô hình Consumer:** Màn hình chỉ chịu trách nhiệm hiển thị (Read). Khi user muốn Sửa/Hủy/Tạo sự kiện, hãy tái sử dụng các Dialog/Form component từ các `BF-ENR-01` và `BF-ENR-02`.
- Sử dụng hàm `getStatusBadgeClass()` từ `src/lib/statusColors.ts` để lấy màu sắc chuẩn cho các trạng thái Booking.
- Có thể dùng thư viện lịch (như FullCalendar hoặc component Calendar của Radix) để hỗ trợ render Overlap event tốt hơn.

### ⛔ Hàng rào An toàn (Guardrails)
- **KHÔNG** viết form "Cập nhật Booking Test" hoặc "Tạo Học thử" trực tiếp vào file của màn hình Lịch này. Hãy import chúng từ thư mục chức năng gốc.
- **KHÔNG** tự hardcode màu sắc thẻ lịch.
