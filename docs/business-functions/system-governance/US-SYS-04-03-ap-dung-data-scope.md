...
| Load data từng menu | Mỗi menu khi tải dữ liệu phải đi qua rule data access chung đã được thực thi. |
...
- Không định nghĩa lại topic, role hoặc permission matrix; phần đó thuộc `US-SYS-04-01`.
- Không gán role cho user; phần đó thuộc `US-SYS-04-02`.
- Không xử lý login; phần đó thuộc `US-SYS-05-01`.
- Không thay thế bảo mật phía hệ thống nền bằng bộ lọc giao diện. Giao diện chỉ được xem là lớp hỗ trợ trải nghiệm/mock, không phải lớp bảo mật chính.

---

## 4. Preconditions

- `US-SYS-04-01` đã có role, permission matrix và data scope hợp lệ.
- `US-SYS-04-02` đã có user và role assignment active.
- `US-SYS-05-01` đã nạp đúng effective permission và effective scope vào session.
...
6. Hệ thống tải dữ liệu module qua lớp kiểm soát truy cập dữ liệu đã thực thi authorization.
...
| R-11 | Bộ lọc giao diện chỉ dùng để hỗ trợ trải nghiệm và mock data; hệ thống nền vẫn phải thực thi cùng rule khi có dịch vụ thật. |
...
| C-09 | Bộ lọc giao diện và bộ lọc hệ thống nền cho kết quả khác nhau | Hệ thống nền là nguồn quyết định; giao diện phải đồng bộ lại hoặc hiển thị lỗi dữ liệu không hợp lệ. |
...
- [ ] **AC-14** Data access rule có thể verify ở hệ thống nền hoặc lớp kiểm soát truy cập bằng test case cho scope, cơ sở và sharing.
- [ ] **AC-15** Giao diện không được hiển thị record ngoài kết quả đã được lớp kiểm soát truy cập cho phép.

---

## 10. Dependencies

- `US-SYS-04-01` cung cấp role, permission matrix và scope của role.
- `US-SYS-04-02` cung cấp user và role assignment.
- `US-SYS-05-01` nạp user, effective permission và effective scope sau login.
