...
* **Bối cảnh:** Trước đây, khi học viên xin nghỉ học buổi chính thức và có nhu cầu học bù, nhân viên chăm sóc và giáo viên phải trao đổi thủ công qua tin nhắn nhóm hoặc lập ticket hỗ trợ. Quy trình này dẫn đến thiếu minh bạch trong việc theo dõi quota học bù, theo dõi sĩ số lớp bù và đồng bộ kết quả điểm danh.
* **Vấn đề hiện tại:** 
...
* **Mục tiêu & Giá trị mang lại:** 
  ...
  - Phân định rõ 2 kịch bản học bù: 
    - **Trường hợp 1 (Hủy lớp do trung tâm):** Tạo một buổi học bù riêng cho cả lớp, học viên tham gia bị trừ 1 buổi quota như buổi học thường.
    - **Trường hợp 2 (Học bù ghép cá nhân):** Nhét học viên nghỉ lẻ vào một lớp chính khác cùng trình độ. Không tính phí/không trừ thêm quota học viên, quản lý như học viên học thử (Guest/Trial) trong danh sách điểm danh.
...
* **Khó khăn lớn nhất (Pain-points):** Mất thời gian kiểm tra thủ công xem lớp khác cùng trình độ còn slot trống không; dễ quên kiểm tra việc học viên đã từng học bù cho buổi nghỉ đó chưa.
...
* **Ràng buộc nghiệp vụ toàn cục (Global Rules):**
  ...
  - **[RULE-MAKEUP-03] Quy tắc trừ Quota & Học phí:** 
    - Đối với buổi học chính bị nghỉ: Học viên bị trừ 1 buổi học (quota) theo đúng quy tắc gói học.
    - Đối với buổi học bù ghép (Trường hợp 2): **Không tính phí, không trừ thêm quota** của học viên.
    - Đối với buổi học bù mở riêng do trung tâm hủy lớp (Trường hợp 1): Học viên tham gia sẽ bị trừ 1 buổi quota (thay cho buổi hủy chưa trừ).
...
sequenceDiagram
    ...
    participant F as Màn hình Quản lý Học bù (/app/makeup_schedule)
    ...
    F-->>U: Hiển thị bảng danh sách phiếu học bù
    
    U->>F: Nhấn nút [Tạo đơn học bù]
    ...
    S-->>F: Phản hồi sĩ số (Cảnh báo Overbooking nếu đầy slot)
    U->>F: Nhấn [Xác nhận lưu]
...
|------------|---------------|------------------|----------------------------------|-------------------|
| Bộ lọc Cơ sở | Ô chọn danh sách | Cơ sở hiện tại | Lọc danh sách phiếu học bù theo chi nhánh | Thu gọn danh sách |
| Bộ lọc Loại học bù | Ô chọn danh sách | Tất cả | Cho phép chọn: `Học bù ghép lớp` hoặc `Lớp bù riêng` | Thu gọn danh sách |
| Ô tìm kiếm nhanh | Ô nhập chữ | Trống | Tìm theo Tên học viên, Mã học viên, Tên lớp gốc | Đầy đủ |
...
|---------------|---------------|----------------|------------------------------------------------|-------------------|
| **Mã phiếu & Học viên** | Chữ đậm + mã mờ | Mã phiếu & Tên học viên | Hiển thị tên học viên kèm mã phiếu học bù | Giữ nguyên trên di động |
| **Lớp chính & Buổi nghỉ** | Văn bản thường | Thông tin lớp gốc | Hiển thị tên lớp chính và ngày/giờ buổi nghỉ | Thu gọn hiển thị |
| **Lớp bù & Buổi học bù** | Văn bản chính | Thông tin lớp ghép | Hiển thị tên lớp bù, phòng học và ngày học bù | Ẩn chi tiết phòng |
| **Loại học bù** | Nhãn màu mờ | Trường loại bù | Phân biệt: `Ghép lớp` hoặc `Lớp bù riêng` | Ẩn trên di động |
| **Trạng thái phiếu** | Nhãn màu (Badge) | Trường trạng thái | Sử dụng hàm ánh xạ màu chuẩn trạng thái | Thu gọn thành dấu chấm màu |
| **Hành động** | Nút biểu tượng | Hệ thống | Nút [Xem chi tiết], [Duyệt], [Hủy đơn] | Luôn hiện nút hành động |
...
| Vai trò người dùng | Xem danh sách | Tạo đơn CS | Tạo đơn GV (Nháp) | Phê duyệt đơn | Hủy đơn |
|---|:---:|:---:|:---:|:---:|:---:|
| **Quản trị viên (Admin)** | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Quản lý cơ sở (Branch Manager)** | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Chuyên viên Chăm sóc (CSM)** | ✅ | ✅ | ❌ | ✅ | ✅ |
| **Giáo viên (Teacher)** | ✅ | ❌ | ✅ | ❌ | ❌ |
...
* **Tiêu chí nghiệm thu (Acceptance Criteria):**
  - **AC-1 (Happy Path - Tạo đơn thành công do CS thực hiện):**
    ...
    - **Khi:** Chọn Lớp bù C (ngày 15/10) còn trống slot và nhấn [Lưu phiếu].
  ...
  - **AC-2 (Alternate Path - Cảnh báo Overbooking khi đầy sĩ số):**
    ...
    - **Thì:** Giao diện hiển thị ô cảnh báo màu vàng "Lớp đã đạt sĩ số tối đa (15/15). Bạn có chắc chắn muốn chèn thêm?", nếu bấm [Tiếp tục chèn] thì hệ thống vẫn cho phép lưu phiếu.
  - **AC-3 (Unhappy Path - Chặn buổi nghỉ quá 30 ngày):**
    ...
    - **Thì:** Danh sách không hiển thị buổi nghỉ ngày 01/05 và báo thông báo "Buổi nghỉ đã quá thời hạn 30 ngày quy định học bù".

#### Action 1.2: Phê duyệt đơn học bù do Giáo viên tạo nháp
* **Luồng kích hoạt (Event/Flow):** Khi Giáo viên tạo đơn học bù, phiếu ở trạng thái `Chờ duyệt`. Chuyên viên Chăm sóc / Quản lý mở xem chi tiết phiếu và bấm [Duyệt đơn].
* **Tiêu chí nghiệm thu (Acceptance Criteria):**
...
* **Tiêu chí nghiệm thu (Acceptance Criteria):**
  - **AC-1 (Happy Path - Hủy phiếu thành công):**
    ...
    - **Khi:** Chuyên viên Chăm sóc bấm [Hủy phiếu] và xác nhận trên hộp thoại.
    - **Thì:** Phiếu chuyển sang trạng thái `Từ chối/Hủy`, gỡ tên Học viên E khỏi danh sách điểm danh lớp bù và hoàn lại quyền tạo đơn học bù mới cho học viên (nếu còn trong thời hạn 30 ngày).

---

## 5. ĐẶC TẢ KẾT NỐI HỆ THỐNG (API SPECIFICATION)

*Mô tả luồng trao đổi thông tin giữa giao diện và hệ thống nghiệp vụ phía sau:*

*   **Nghiệp vụ lấy danh sách phiếu học bù:** Giao diện gửi gói yêu cầu lấy danh sách phiếu học bù kèm theo mã cơ sở, khoảng thời gian và mã trạng thái. Hệ thống kiểm tra quyền nhân sự, gọi đến cơ sở dữ liệu phiếu học bù và trả về danh sách kết quả kèm thông tin phân trang.
*   **Nghiệp vụ tạo đơn học bù:** Giao diện gửi gói dữ liệu gồm mã học viên, mã buổi nghỉ, mã buổi học bù được chọn. Hệ thống thực hiện kiểm tra thời hạn 30 ngày, gọi đến nghiệp vụ xếp lớp để ghi nhận học viên vào bảng điểm danh của buổi học bù và lưu phiếu vào cơ sở dữ liệu học viên.
*   **Nghiệp vụ đồng bộ điểm danh tự động:** Khi Giáo viên thực hiện điểm danh buổi học trên giao diện Giáo viên, hệ thống kiểm tra các dòng điểm danh có nhãn `Học bù`. Nếu chọn `Có mặt`, hệ thống gọi đến nghiệp vụ phiếu học bù để cập nhật trạng thái phiếu thành `Hoàn tất`. Nếu chọn `Vắng mặt`, hệ thống cập nhật trạng thái phiếu thành `Đã vắng` (ghi nhận mất lượt bù).
...
- **[CASE-03] Trùng lịch học chính của học viên (Exception Flow - Schedule Conflict):**
  ...
  - *Cách xử lý:* Hệ thống thực hiện kiểm tra trùng lịch và hiển thị cảnh báo đỏ: "Học viên đã có lịch học lớp [Tên lớp] vào cùng khung giờ này. Vui lòng chọn buổi học khác."
...
- **[CASE-05] Thao tác đồng thời trên cùng một slot cuối của lớp bù (Exception Flow - Race Condition):**
  - *Tình huống:* Hai nhân viên chăm sóc cùng mở chọn 1 slot cuối cùng của lớp bù tại cùng một thời điểm.
